...
[![GitHub contributors](https://img.shields.io/github/contributors/nike1v/serverless-next13)](https://github.com/nike1v/serverless-next13/graphs/contributors)
...
[![npm latest](https://img.shields.io/npm/v/@dolsze/serverless-component)](https://www.npmjs.com/package/@dolsze/serverless-component?activeTab=versions)
[![npm alpha](https://img.shields.io/npm/v/@dolsze/serverless-component/alpha)](https://www.npmjs.com/package/@dolsze/serverless-component?activeTab=versions)
![Build Status](https://github.com/nike1v/serverless-next13/workflows/CI/badge.svg)
![End-to-end Tests](https://github.com/nike1v/serverless-next13/workflows/End-to-end%20Tests/badge.svg)
![CircleCI Build Status](https://img.shields.io/circleci/build/github/nike1v/serverless-next13)
[![Codacy Badge](https://app.codacy.com/project/badge/Grade/60824e8ec2d04af6817eca817c807f8f)](https://www.codacy.com/gh/nike1v/serverless-next13/dashboard?utm_source=github.com&utm_medium=referral&utm_content=nike1v/serverless-next13&utm_campaign=Badge_Grade)
[![codecov](https://codecov.io/gh/nike1v/serverless-next13/branch/master/graph/badge.svg)](https://codecov.io/gh/nike1v/serverless-next13)
...
Please review [features](https://github.com/nike1v/serverless-next13#features) for a list of currently supported features.
...
> :warning: This README reflects the latest changes on the `master` branch. It may or may not yet be published to the `latest` (stable) or `alpha` release in npm. Please go to [Releases](https://github.com/nike1v/serverless-next13/releases), find the correct `@dolsze/serverless-component` version you are using, and open the README for that release for more accurate information. If a feature is listed in this README but not working, please first try upgrading to the most recent `alpha` release in npm.
...
This project is a better version of the [serverless plugin](https://github.com/nike1v/serverless-next13/tree/master/packages/deprecated/serverless-plugin) which focuses on addressing core issues like [next 9 support](https://github.com/nike1v/serverless-next13/issues/101), [better development experience](https://github.com/nike1v/serverless-next13/issues/59), [the 200 CloudFormation resource limit](https://github.com/nike1v/serverless-next13/issues/17) and [performance](https://github.com/nike1v/serverless-next13/issues/13).
...
Note that some features may only be on the latest [alpha version](https://www.npmjs.com/package/@dolsze/serverless-component?activeTab=versions). If a feature is listed as supported but not working on the `latest` tag, it most likely is in the `alpha` tag. If you can, please help us test the latest alpha changes and [submit a bug report](https://github.com/nike1v/serverless-next13/issues/new?assignees=&labels=&template=bug_report.md&title=) if you find any issues. Thank you!

Is there a feature that you want but is not yet supported? Please open a [new issue](https://github.com/nike1v/serverless-next13/issues/new?assignees=&labels=&template=feature_request.md&title=) to let us know!
...
- [x] [Next.js 10 Localization](https://nextjs.org/blog/next-10). See: https://github.com/nike1v/serverless-next13/issues/721 for more details and tips. We also automatically detect and copy the default configuration files if you are using the `next-i18next` package.
- [x] [Incremental Static Regeneration](https://nextjs.org/docs/basic-features/data-fetching#incremental-static-regeneration). Requires SQS.SendMessage permissions on your Lambda role and various SQS/Lambda permissions on your deployment user (note, adding ISR to an existing app requires manually updating the Lambda role's permissions for now - see https://github.com/nike1v/serverless-next13/issues/1510). See https://github.com/nike1v/serverless-next13/pull/1028 for more details, big thanks to @kirkness for this amazing work.
...
myNextApplication:
  component: "@dolsze/serverless-component@{version_here}" # it is recommended you pin the latest stable version of serverless-next.js
```

:no_entry_sign: **If you specify `@dolsze/serverless-component` in your `serverless.yml` file, do not add `@dolsze/serverless-component` to your package.json file, it is not used and only the version in `serverless.yml` file is used, which Serverless pulls from npm by itself. If you do not specify the version, it will use the `latest` tag, which refers to the latest stable version [here](https://www.npmjs.com/package/@dolsze/serverless-component) (i.e not alpha versions).**
...
myNextApplication:
  component: "./node_modules/@dolsze/serverless-component"
```
...
myNextApplication:
  component: "@dolsze/serverless-component@{version_here}"
  inputs:
...
To specify your own CloudFront inputs, just add any [aws-cloudfront inputs](https://github.com/nike1v/serverless-next13/tree/master/packages/serverless-components/aws-cloudfront#3-configure) under `cloudfront`:
...
You can also [update an existing cloudfront distribution](https://github.com/nike1v/serverless-next13/tree/master/packages/serverless-components/aws-cloudfront#updating-an-existing-cloudfront-distribution) using custom cloudfront inputs.
...
| build.outputStandalone       | `boolean`         | `false`                                                            | **Experimental** Use the Next.js 12 [Output File Tracing](https://nextjs.org/docs/advanced-features/output-file-tracing) for copying required files into the Lambdas. This option doesn't set any `target` and instead relies on the default value, because `target` is now deprecated.                                                                                                                                                                                              |
| cloudfront                   | `object`          | `{}`                                                               | Inputs to be passed to [aws-cloudfront](https://github.com/serverless-components/aws-cloudfront)                                                                                                                                                                                                                                                                                                                                                                                     |
| certificateArn               | `string`          | ``                                                                 | Specific certificate ARN to use for CloudFront distribution. Helpful if you have a wildcard SSL cert you wish to use. This currently works only in tandem with the`domain`input. Please check [custom CloudFront configuration](https://github.com/nike1v/serverless-next13#custom-cloudfront-configuration) for how to specify`certificate`without needing to use the`domain`input (note that doing so will override any certificate due to the domain input).                      |
...
This is likely either because of a Lambda@Edge code size issue (see above for potential solutions. Related [GitHub Issue](https://github.com/nike1v/serverless-next13/issues/611)) or if you have a slow network upload speed and/or are trying to deploy a large Lambda package.
...
#### Should I use the [serverless-plugin](https://github.com/nike1v/serverless-next13/tree/master/packages/deprecated/serverless-plugin) or this component?
...
See `examples/dynamodb-crud` for an example Todo application that interacts with DynamoDB. You can find a full list of examples [here](https://github.com/nike1v/serverless-next13/tree/master/packages/serverless-components/nextjs-component/examples)
...
2. Alternatively you could use S3 to store the `.serverless` files, see an example [here](https://gist.github.com/hadynz/b4e190e0ce10e5811cb462920a9c678f), [here](https://gist.github.com/dphang/7395ee09f6182f6b34f224660bed8e8c) (uses multiple `serverless.yml` files), or [here](https://github.com/nike1v/serverless-next13/issues/328#issuecomment-655466654) (GitHub Actions-based, uses multiple `serverless.yml` files).
...
2. Follow https://github.com/nike1v/serverless-next13/issues/530#issuecomment-680122810 and set a `serverless.yml` variable first, then concatenate:

```yml
stage: ${env.STAGE}
my-app:
  component: "@dolsze/serverless-component@1.18.0"
  inputs:
    domain:
      - "${stage}-front-end"
      - mydomain.com
...
Refer to the [README](https://github.com/nike1v/serverless-next13/blob/master/packages/libs/lambda-at-edge/src/build/third-party/README.md) for more information and caveats.

## Reporting Issues

You can open a new issue [here](https://github.com/nike1v/serverless-next13/issues). If posting a problem, please follow the [debugging wiki](https://github.com/nike1v/serverless-next13/wiki/Debugging-Issues) first for some useful tips, and try to include as much information to reproduce the issue.

If you are reporting a security vulnerability, please follow the [security policy](https://github.com/nike1v/serverless-next13/security/policy) instead.
...
<a href="https://github.com/nike1v/serverless-next13/graphs/contributors">
<img src="https://contrib.rocks/image?repo=nike1v/serverless-next13" />
</a>
