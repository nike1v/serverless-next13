...
export default class CoreBuilder {
    ...
    getDefaultHandlerFileFilter(hasAPIRoutes, pageManifest) {
        return (file) => {
            ...
            const isNotExcludedJSFile = hasAPIRoutes ||
                ...
                this.isSSRJSFile(pageManifest, pathToPosix(path.relative(path.join(this.serverDir, "pages"), file)) // important: make sure to use posix path to generate forward-slash path across both posix/windows
                );
    ...
    async copyChunks(handlerDir) {
        return (await fse.pathExists(join(this.serverDir, "chunks")))
            ? fse.copy(join(this.serverDir, "chunks"), join(this.outputDir, handlerDir, "chunks"))
            : Promise.resolve();
    }
    /**
    ...
    async copyJSFiles(handlerDir) {
        await Promise.all([
            (await fse.pathExists(join(this.serverDir, "webpack-api-runtime.js")))
                ? fse.copy(join(this.serverDir, "webpack-api-runtime.js"), join(this.outputDir, handlerDir, "webpack-api-runtime.js"))
                : Promise.resolve(),
            (await fse.pathExists(join(this.serverDir, "webpack-runtime.js")))
                ? fse.copy(join(this.serverDir, "webpack-runtime.js"), join(this.outputDir, handlerDir, "webpack-runtime.js"))
                : Promise.resolve()
        ]);
    }
