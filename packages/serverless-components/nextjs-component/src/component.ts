...
} from "@cryptodiffer-sls-next/s3-static-assets";
import {
  createInvalidation,
  checkCloudFrontDistributionReady
} from "@cryptodiffer-sls-next/cloudfront";
...
import { removeLambdaVersions } from "@cryptodiffer-sls-next/aws-lambda/dist/removeLambdaVersions";
...
class NextjsComponent extends Component {
  ...
  async deploy(
    ...
    const [
      ...
      this.load("@cryptodiffer-sls-next/aws-s3"),
      this.load("@cryptodiffer-sls-next/aws-cloudfront"),
      this.load("@cryptodiffer-sls-next/aws-sqs"),
      this.load("@cryptodiffer-sls-next/aws-lambda", "defaultEdgeLambda"),
      this.load("@cryptodiffer-sls-next/aws-lambda", "apiEdgeLambda"),
      this.load("@cryptodiffer-sls-next/aws-lambda", "imageEdgeLambda"),
      this.load("@cryptodiffer-sls-next/aws-lambda", "regenerationLambda")
    ]);
    ...
    if (domain && subdomain) {
      const domainComponent = await this.load("@cryptodiffer-sls-next/domain");
  ...
  async remove(): Promise<void> {
    const [bucket, cloudfront, sqs, domain] = await Promise.all([
      ...
      this.load("@cryptodiffer-sls-next/aws-cloudfront"),
      this.load("@cryptodiffer-sls-next/aws-sqs"),
      this.load("@cryptodiffer-sls-next/domain")
    ]);
