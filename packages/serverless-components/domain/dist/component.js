...
class Domain extends core_1.Component {
    async default(inputs = {}) {
        ...
        if (!(0, utils_1.isMinimumProtocolVersionValid)(inputs.domainMinimumProtocolVersion)) {
            throw Error('"minimumProtocolVersion" has invalid value.');
        }
        const clients = (0, utils_1.getClients)(this.context.credentials.aws, inputs.region);
        this.context.debug("Formatting domains and identifying cloud services being used.");
        const subdomains = (0, utils_1.prepareSubdomains)(inputs);
        ...
        const domainHostedZoneId = await (0, utils_1.getDomainHostedZoneId)(clients.route53, inputs.domain, inputs.privateZone);
        let certificateArn = "";
        if (inputs.certificateArn === "") {
            this.context.debug(`Searching for an AWS ACM Certificate based on the domain: ${inputs.domain}.`);
            certificateArn = await (0, utils_1.getCertificateArnByDomain)(clients.acm, inputs.domain);
        }
        else {
            this.context.debug(`Using specified SSL Certificate ARN: ${inputs.certificateArn}.`);
        ...
        if (!certificateArn) {
            ...
            certificateArn = await (0, utils_1.createCertificate)(clients.acm, inputs.domain);
        }
        this.context.debug(`Checking the status of AWS ACM Certificate.`);
        const certificate = await (0, utils_1.describeCertificateByArn)(clients.acm, certificateArn);
        if (certificate.Status === "PENDING_VALIDATION") {
            ...
            await (0, utils_1.validateCertificate)(clients.acm, clients.route53, certificate, inputs.domain, domainHostedZoneId);
            this.context.log("Your AWS ACM Certificate has been created and is being validated via DNS.  This could take up to 30 minutes since it depends on DNS propagation. Continuing deployment, but you may have to wait for DNS propagation.");
        }
        ...
        for (const subdomain of subdomains) {
            if (subdomain.type === "awsS3Website") {
                ...
                this.context.debug(`Adding ${subdomain.domain} domain to CloudFront distribution with URL "${subdomain.url}.\nTLS minimum protocol version: ${inputs.domainMinimumProtocolVersion}`);
                await (0, utils_1.addDomainToCloudfrontDistribution)(clients.cf, subdomain, certificate.CertificateArn, inputs.domainMinimumProtocolVersion, inputs.domainType, inputs.defaultCloudfrontInputs, this.context);
                this.context.debug(`Configuring DNS for distribution "${subdomain.url}".`);
                if (domainHostedZoneId) {
                    await (0, utils_1.configureDnsForCloudFrontDistribution)(clients.route53, subdomain, domainHostedZoneId, subdomain.url.replace("https://", ""), inputs.domainType, this.context);
                }
                else {
                    this.context.debug(`DNS records for domain ${subdomain.domain} were not configured because the domain was not found in your account. Please configure the DNS records manually`);
                }
            }
    ...
    async remove() {
        ...
        const domainHostedZoneId = await (0, utils_1.getDomainHostedZoneId)(clients.route53, this.state.domain, this.state.privateZone);
        for (const subdomain in this.state.subdomains) {
            ...
            if (domainState.type === "awsS3Website") {
                ...
                await (0, utils_1.removeDomainFromCloudFrontDistribution)(clients.cf, domainState, this.state.domainMinimumProtocolVersion, this.context);
                if (domainHostedZoneId) {
                    this.context.debug(`Removing CloudFront DNS records for domain ${domainState.domain}`);
                    await (0, utils_1.removeCloudFrontDomainDnsRecords)(clients.route53, domainState.domain, domainHostedZoneId, domainState.url.replace("https://", ""), this.context);
                }
                else {
                    this.context.debug(`Could not remove the DNS records for domain ${domainState.domain} because the domain was not found in your account. Please remove the DNS records manually`);
                }
            }
